...
// it is not curusial because we are caching results, we will double check in the client side
...
//         eq(businessWebsiteMapTable.websiteId, websiteId),
//         gte(businessDeliveryZoneMapTable.lat, minLat),
//         lte(businessDeliveryZoneMapTable.lat, maxLat),
//         gte(businessDeliveryZoneMapTable.lng, minLng),
//         lte(businessDeliveryZoneMapTable.lng, maxLng),
//     ];

//     const deliveryConditions = [
//         gte(businessDeliveryZoneMapTable.maxLat, lat),
//         lte(businessDeliveryZoneMapTable.minLat, lat),
//         gte(businessDeliveryZoneMapTable.maxLng, lng),
//         lte(businessDeliveryZoneMapTable.minLng, lng),
//     ];
...
//     const [countResult, nearbyBusinesses] = await Promise.all([
//         db
//             .select({
//                 count: count()
//             })
...
//             .innerJoin(
//                 businessWebsiteMapTable,
//                 eq(businessesTable.id, businessWebsiteMapTable.businessId)
//             )
//             .innerJoin(
//                 businessDeliveryZoneMapTable,
//                 eq(businessesTable.id, businessDeliveryZoneMapTable.businessId)
//             )
//             .where(and(...whereConditions)),
...
//                 lat: businessDeliveryZoneMapTable.lat,
//                 lng: businessDeliveryZoneMapTable.lng,
...
//                 distance: sql<number>`${radius} * 2 * ASIN(
//                 SQRT(
//                     0.5 - COS((${businessDeliveryZoneMapTable.lat} - ${lat}) * 0.017453292519943295) / 2 + 
//                     COS(${lat} * 0.017453292519943295) * COS(${businessDeliveryZoneMapTable.lat} * 0.017453292519943295) * 
//                     (1 - COS((${businessDeliveryZoneMapTable.lng} - ${lng}) * 0.017453292519943295)) / 2
//                 )
//             )`.mapWith(Number).as('distance'),
//                 /////////////////////////
//             })
