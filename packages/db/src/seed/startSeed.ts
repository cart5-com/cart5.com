...
import { addToTeamSeed } from "./addToTeamSeed";
...
const startSeed = async () => {
    ...
    const cart5DevWebsite = await createWebsite_Service(cart5DevAdminUser?.id!, "cart5dev", null);
    ...
    const obiteWebsite = await createWebsite_Service(cuneytObite?.id!, "obite", cart5Team_asSupportTeam?.teamId!);
    ...
    await addToTeamSeed(obiteWebsite.ownerTeamId, thushObite?.id!, [TEAM_PERMISSIONS.FULL_ACCESS]);
    ...
    const flamesWebsite = await createWebsite_Service(thushObite?.id!, "flames", obiteTeam_asSupportTeam?.teamId!);
    ...
    const flamesRestaurantAdminUser = await createSeedUser("admin@flames.com", password, "Flames Admin");
    await addToTeamSeed(flamesWebsite.ownerTeamId, flamesRestaurantAdminUser?.id!, [TEAM_PERMISSIONS.FULL_ACCESS]);
}
