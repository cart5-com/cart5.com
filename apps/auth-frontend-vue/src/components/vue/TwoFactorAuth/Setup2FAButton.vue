...
import RecoveryCodeDialog from '@src/components/vue/TwoFactorAuth/RecoveryCodeDialog.vue';
...
const setupTwoFactorAuthentication = async () => {
    ...
    const { data, error } = await (await getAuthApiClient().api["two-factor-auth"].new.$post()).json();
