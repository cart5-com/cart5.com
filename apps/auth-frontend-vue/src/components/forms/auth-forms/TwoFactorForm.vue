...
async function onSubmit(values: z.infer<typeof schema>) {
    await withSubmit(async () => {
        const { data, error } = await (await getAuthApiClient().api_auth["two_factor_auth"].verify.$post({
            form: {
...
async function resetWithRecoveryCode() {
    ...
    await withSubmit(async () => {
        const { data, error } = await (await getAuthApiClient().api_auth["two_factor_auth"]['remove-2fa-with-recovery-code'].$post({
            form: {
                recoveryCode,
                turnstile: await showTurnstile(import.meta.env.VITE_PUBLIC_TURNSTILE_SITE_KEY)
            },
