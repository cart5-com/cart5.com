...
const setupTwoFactorAuthentication = async () => {
    ...
    const { data, error } = await (await authApiClient.two_factor_auth.new.$post()).json();
...
const getRecoveryCode = async () => {
    const { data, error } = await (await authApiClient.two_factor_auth["get-recovery-code"].$post({
        form: {
...
const generateNewRecoveryCode = async () => {
    const { data, error } = await (await authApiClient.two_factor_auth["generate-new-recovery-code"].$post({
        form: {
            turnstile: await showTurnstile(import.meta.env.VITE_PUBLIC_TURNSTILE_SITE_KEY)
        }
    })).json();
...
const removeTwoFactorAuthentication = async () => {
    if (confirm("Are you sure you want to remove two factor authentication?")) {
        const { data, error } = await (await authApiClient.two_factor_auth["remove-2fa"].$post({
            form: {
                turnstile: await showTurnstile(import.meta.env.VITE_PUBLIC_TURNSTILE_SITE_KEY)
            }
        })).json();
