...
import { getPendingTeamInvitations_Service } from "@db/services/team.service";
...
>) => {
    ...
    const pendingInvitations = (await getPendingTeamInvitations_Service(website?.ownerTeamId ?? ''))
        .map(invitation => ({
            ...invitation,
