...
import { currentStore, currentStoreId } from '@dashboard-spa-vue/stores/MyStoresStore';
...
const apiPath = dashboardApiClient.store[':storeId'].team.$get
...
const transferOwnership = async () => {
    ...
    try {
        const response = await dashboardApiClient.store[':storeId'].team_transfer_ownership.$post({
            param: {
...
const removeMember = async () => {
    ...
    try {
        const response = await dashboardApiClient.store[':storeId'].team_remove_member.$post({
            param: {
                storeId: currentStoreId.value ?? ''
            },
            json: {
...
const updatePermissions = async (permissions: string[]) => {
    ...
    try {
        const { error } = await (await dashboardApiClient.store[':storeId'].team_update_permissions.$post({
            param: {
                storeId: currentStoreId.value ?? ''
            },
            json: {
    ...
    <Card class="max-w-lg mx-auto">
        <CardHeader class="pb-2">
            <p class="text-sm text-muted-foreground">
                ...
                <strong>Store:</strong> <span class="font-medium">{{ currentStore?.name }}</span>
                <span v-if="currentStore?.address1"
                      class="text-muted-foreground ml-1">({{ currentStore?.address1 }})</span>
                <br>
                <span class="mt-1 block">Team members can access and manage this store based on their assigned
                    permissions.</span>
            </p>
        </CardHeader>
        <CardContent>
